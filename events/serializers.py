...
class ActorSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        default = {"role": validated_data["role"]}
        object, create = self.Meta.model.objects.get_or_create(
            event=instance, role=validated_data["role"], defaults=default)

        qs = object.actor.all()
        old = set([item.name for item in qs])
        new = set([item["name"] for item in validated_data["actor"]])

        actors = validated_data.pop("actor")
        actor_models = []
        info = model_meta.get_field_info(self.Meta.model)
        RelatedModel = info.relations["actor"].related_model

        # TODO: this needs to be done better
        actors_l = []
        for actor in actors:
            actor = RelatedModel.objects.get(name=actor["name"])
            actors_l.append(actor.id)

        rm = old.difference(new)
        for item in rm:
            delete = RelatedModel.objects.get(name=item)
            object.actor.remove(delete)

        for item in actors_l:
            object.actor.add(item)
        instance.save()
        return object
